        ...
        <tbody>
            {hero && hero.map((el, i)=>(
                <tr key={`value_hero_${i}`}>
                    ...
                    <td className='flex justify-between px-2 items-center gap-2'>{!el.campaign_id? <Button onClick={()=> add_Campaign(el.id)} className='' size='sm'>Ingaggia</Button>: el.campaign_id === id_campaign? <Button onClick={()=> No_Campaign(el.id)} variant='outline-warning' size='sm'>Congeda</Button> : <Button onClick={()=> add_Campaign(el.id)} size='sm'>Richiama e ingaggia</Button>} <Button  size='sm' variant='outline-danger'>Cancella</Button></td>
                </tr>
            ))}
        </tbody>
      </Table>
